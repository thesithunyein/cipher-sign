...
# SKIP_BINDINGS=1 reuses an already-generated autogen.go (e.g. Docker without forge).
step 0 "Generate Go bindings"
if [[ "${SKIP_BINDINGS:-0}" == "1" ]]; then
    [[ -f "$PROJECT_DIR/go/tools/pkg/contracts/sign/autogen.go" ]] \
        || die "SKIP_BINDINGS=1 but go/tools/pkg/contracts/sign/autogen.go is missing"
    log "SKIP_BINDINGS=1 — reusing existing Go bindings"
else
